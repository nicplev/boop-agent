      ...
      try {
        ...
        if (phones.length === 1) {
          ...
          const { pickedNumber } = await prompts({
            type: "select",
            name: "pickedNumber",
            message: "You have multiple Sendblue numbers — which one should Lumi Assistant reply from?",
...
async function main() {
  banner("Lumi Assistant setup");

  console.log(`
What this does:
  1. Pulls your Sendblue keys (via their CLI, or you paste them)
  2. Asks whether Lumi Assistant should use your Claude Code or Codex subscription
...
`);
  ...
  const answers = await prompts(
    [
      ...sendbluePrompts,
      {
        type: "select",
        name: "BOOP_RUNTIME",
        message: "Which subscription should Lumi Assistant use for the agent runtime?",
        choices: [
          {
      ...
      },
      {
        ...
        message: "How much Codex reasoning effort should Lumi Assistant use?",
  ...
  const inferredCurrent = existingVoyage
    ...
    : existingOpenai
      ...
      : "local";
  console.log(`
Lumi Assistant's recall() searches your stored memories by semantic similarity. Pick
how you want to generate embeddings:
  ...
  • Local  — free, runs in-process via @huggingface/transformers
            ...
            ~1.3GB and caches in Lumi Assistant's local data folder. No API key.
...
`);
  ...
  console.log(`
Lumi Assistant can optionally expose a local Patchright Chrome profile to spawned agents.
...
Lumi Assistant reads the Codex credentials saved on disk. Set BOOP_CODEX_AUTH_HOME in
.env.local only if you need Lumi Assistant to read a different Codex home containing
auth.json.
