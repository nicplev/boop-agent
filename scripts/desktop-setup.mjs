...
async function copyToApplicationsIfWanted(appPath) {
  ...
  try {
    const answer = await rl.question("\nCopy Lumi Assistant.app to /Applications now? [Y/n] ");
    if (answer.trim().toLowerCase().startsWith("n")) return;
  } finally {
    rl.close();
  }

  const destination = "/Applications/Lumi Assistant.app";
...
async function main() {
  console.log(`
Lumi Assistant desktop setup

This command prepares the desktop app runtime folder, runs Lumi Assistant's existing setup
...
`);
  ...
  logStep("Running Lumi Assistant setup in the desktop runtime");
  ...
  const appPath =
    process.platform === "darwin"
      ? join(root, "dist", process.arch === "arm64" ? "mac-arm64" : "mac", "Lumi Assistant.app")
      : join(root, "dist");
...
Launch Lumi Assistant from the app bundle, then keep it in the Dock if you want it handy.
`);
}
