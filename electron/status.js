...
function displayMessage(status) {
  ...
  if (
    ...
    return "Lumi Assistant is stopped.";
  }
  return message || "Waiting for Lumi Assistant.";
}
...
function render(status) {
  ...
  fields.emptyTitle.textContent =
    status.state === "setup-required"
      ? "Setup required"
      : starting
        ? "Starting Lumi Assistant"
        : status.state === "error"
          ? "Lumi Assistant stopped"
          : "Lumi Assistant is stopped";
...
function optimisticForAction(action) {
  if (action === "stop") {
    return {
      ...
      webhookDetails: "",
      webhookCheckedAt: "",
      lastMessage: "Lumi Assistant is stopped.",
    };
  }
  ...
  return {
    ...
    webhookDetails: "",
    webhookCheckedAt: "",
    lastMessage: action === "restart" ? "Restarting Lumi Assistant." : "Starting Lumi Assistant.",
  };
}
