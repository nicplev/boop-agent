...
function statusMenuTemplate() {
  ...
  return [
    { label: `Lumi Assistant: ${plainStatus(status.state)}`, enabled: false },
    { type: "separator" },
    ...
    { label: "Show Lumi Assistant", click: loadStatusPage },
    {
      label: "Check Sendblue Webhook",
      enabled: Boolean(status.expectedWebhookUrl || status.publicUrl),
      click: checkSendblueWebhook,
    },
    { label: "Start Lumi Assistant", enabled: !boopProcess && !starting, click: startBoop },
    { label: "Restart Lumi Assistant", enabled: !starting, click: restartBoop },
    { label: "Stop Lumi Assistant", enabled: canStop, click: stopBoop },
    { type: "separator" },
...
function ingestLine(line) {
  ...
  if (/lumi-assistant server listening on :/.test(plain)) next.server = "running";
  ...
  child.on("exit", (code) => {
    ...
    if (code === 0 || status.state === "setup-required") {
      ...
      setStatus({
        ...
        convex: "stopped",
        dashboard: "stopped",
        lastMessage: `Lumi Assistant exited with code ${code}`,
      });
    }
  });
}
...
function stopBoop() {
  ...
  intentionalStop = true;
  resetServiceStatuses("stopped");
  setStatus({ lastMessage: "Lumi Assistant is stopped." });
  child.kill("SIGTERM");
}
...
function restartBoop() {
  ...
  setStatus({
    ...
    tunnel: "unknown",
    publicUrl: "",
    lastMessage: "Restarting Lumi Assistant.",
  });
