...
Lumi Assistant is a small distributed system disguised as a single-server app. Four moving parts, each doing one job.
...
Lumi Assistant delegates all third-party integrations to [Composio](https://composio.dev/?utm_source=chris&utm_medium=youtube&utm_campaign=collab). One SDK, 1000+ toolkits, hosted auth.
...
Security model:
- Lumi Assistant does not store third-party passwords or OAuth tokens for local browser use. Browser cookies and sessions live in the chosen Chrome profile directory.
...
- **Skills library omitted** — too Lumi Assistant-specific; write your own prompts/policies in `server/*-agent.ts` system prompts.
